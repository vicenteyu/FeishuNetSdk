// ************************************************************************
// Assembly         : FeishuNetSdk
// Author           : yxr
// Created          : 2026-05-30
//
// Last Modified By : yxr
// Last Modified On : 2026-05-29
// ************************************************************************
// <copyright file="Test_PostMinutesV1MinutesUnsubscriptionBodyDto.cs" company="Vicente Yu">
//     MIT
// </copyright>
// <summary>测试 取消订阅妙记变更事件 请求体</summary>
// ************************************************************************
namespace FeishuNetSdk.Tests.Minutes;

/// <summary>
/// 测试 取消订阅妙记变更事件 请求体
/// <para>接口ID：7644841241634032587</para>
/// <para>文档地址：https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/minutes-v1/minute/unsubscription</para>
/// <para>JSON地址：https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2fuAjLw4CM%2fukTMukTMukTM%2fminutes-v1%2fminute%2funsubscription</para>
/// </summary>
[TestClass]
public class Test_PostMinutesV1MinutesUnsubscriptionBodyDto : TestBase
{
    [TestMethod]
    public void TestDto()
    {
        var json = """
{
    "event_type": "minutes.minute.generated_v1"
}
""";
        var result = Deserialize<FeishuNetSdk.Minutes.PostMinutesV1MinutesUnsubscriptionBodyDto>(json);
        Assert.IsNotNull(result);
    }
}
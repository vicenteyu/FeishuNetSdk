// ************************************************************************
// Assembly         : FeishuNetSdk
// Author           : yxr
// Created          : 2025-06-24
//
// Last Modified By : yxr
// Last Modified On : 2025-06-24
// ************************************************************************
// <copyright file="Test_PutHireV1TalentsByTalentIdExternalInfoResponseDto.cs" company="Vicente Yu">
//     MIT
// </copyright>
// <summary>测试 更新人才外部信息 响应体</summary>
// ************************************************************************
namespace FeishuNetSdk.Tests.Hire;

/// <summary>
/// 测试 更新人才外部信息 响应体
/// <para>接口ID：7045100014603550721</para>
/// <para>文档地址：https://open.feishu.cn/document/server-docs/hire-v1/get-candidates/import-external-system-information/update</para>
/// <para>JSON地址：https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2fukTMukTMukTM%2fuMzM1YjLzMTN24yMzUjN%2fhire-v1%2ftalent-external_info%2fupdate</para>
/// </summary>
[TestClass]
public class Test_PutHireV1TalentsByTalentIdExternalInfoResponseDto : TestBase
{
    [TestMethod]
    public void TestDto()
    {
        var json = """
{
    "code": 0,
    "msg": "ok",
    "data": {
        "external_info": {
            "talent_id": "7043758982146345223",
            "external_create_time": "1608467675393"
        }
    }
}
""";
        var result = Deserialize<FeishuResponse<FeishuNetSdk.Hire.PutHireV1TalentsByTalentIdExternalInfoResponseDto>>(json);
        Assert.IsNotNull(result);
    }
}
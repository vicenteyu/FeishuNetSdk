...
public record PostInteractiveV1CardUpdateBodyDto
{
    ...
    /// <summary>
    /// <para>消息卡片的 JSON 结构内容。具体结构说明可参见[卡片 JSON 结构](https://open.feishu.cn/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-v2-structure)。</para>
    ...
    /// <para>**注意**：</para>
    /// <para>- 如果卡片为共享卡片（卡片 JSON 结构 `config` 参数内 `"update_multi": true` 表示共享卡片），则无需传入该值，默认会更新所有用户接收到的卡片内容。</para>

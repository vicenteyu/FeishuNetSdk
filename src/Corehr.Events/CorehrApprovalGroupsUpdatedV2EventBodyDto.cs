...
/// <summary>
/// 组织架构调整状态变更事件 事件体
/// <para>组织架构调整状态变更事件</para>
/// <para>- 当用户在『飞书人事-我的团队/人员管理-组织架构』，查看调整链接可以获取到 该用户发起的所有组织架构调整， 进入可找到审批流程。</para>
/// <para>- 当该审批单状态发生变更后， 用户会收到流程状态变更事件。</para>
/// <para>- 延迟说明：数据库主从延迟2s以内，即：用户接收到流程状态变更消息后2s内调用查询状态接口可能查不到变更信息。{使用示例}(url=/api/tools/api_explore/api_explore_config?project=corehr&amp;version=v2&amp;resource=approval_groups&amp;event=updated)</para>
...
public record CorehrApprovalGroupsUpdatedV2EventBodyDto() : EventBodyDto("corehr.approval_groups.updated_v2")
{
    ...
    /// <para>**可选值有**：</para>
    /// <para>1:审批中， 流程成功发起，并等待审批人审批。 可以通过『飞书人事-审批-我发起的』 / 『飞书人事-我的团队/人员管理-组织架构-调整记录』 找到审批单据。 ,2:审批通过，该单据已通过审批， 调整记录等待写入。 一方面，组织架构调整支持拆单功能， 同一个调整可能发起多个审批， 当前审批单可能依赖其他审批通过才能写入。,3:已完成</para>
    /// <para>==该状态不代表调整的记录生效完成== 由于记录可能是未来生效， 因此记录的状态需通过 人员异动变更事件 和 部门变更事件获取。</para>
    /// <para>- 人员异动变更事件：[飞书人事-异动-事件](/ssl:/ttdoc/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/job_change/job-change-events)</para>
    /// <para>- 部门变更事件: [飞书人事-组织管理-事件](/ssl:/ttdoc/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/department/events/created),4:已拒绝： 审批未通过。,5:已撤销，用户主动撤销审批， 流程会进入已撤销状态。</para>
    /// <para>必填：否</para>
    ...
    /// <para>==该状态不代表调整的记录生效完成==</para>
    /// <para>由于记录可能是未来生效， 因此记录的状态需通过 人员异动变更事件 和 部门变更事件获取。</para>
    /// <para>- 人员异动变更事件:[飞书人事-异动-事件](/ssl:/ttdoc/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/job_change/job-change-events)</para>
    /// <para>- 部门变更事件: [飞书人事-组织管理-事件](/ssl:/ttdoc/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/department/events/created) ,3:已撤销，用户主动撤销审批， 流程会进入已撤销状态。</para>
    /// <para>必填：否</para>
    /// <para>可选值：<list type="bullet">
    /// <item>1：审批中，流程成功发起，并等待审批人审批。 可以通过『飞书人事-审批-我发起的』 / 『飞书人事-我的团队/人员管理-组织架构-调整记录』 找到审批单据。</item>
    /// <item>2：已完成，</item>
